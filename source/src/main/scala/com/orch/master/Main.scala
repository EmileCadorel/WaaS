...
object Main {
    ...
    def props (name : String, addr : String, port : Int, eaddr : String, eport : Int, bw : Int, sched : String) : Props = Props (new Master (name + s"@${addr}_${port}", addr, port, eaddr, eport, bw, sched))
    ...
    def main (args : Array[String]) {
        ...
        val masterActor = system.actorOf (Main.props (config.name, config.addr, config.port, config.eaddr, config.eport, config.bw, config.sched), name="master")
    }
}
